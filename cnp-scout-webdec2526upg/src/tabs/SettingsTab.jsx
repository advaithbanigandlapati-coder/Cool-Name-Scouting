...
export default function SettingsTab({ settings, setSettings, mine, setMine, setToast }) {
  ...
  async function loadHeaders() {
    if (!local.csvUrl) {
      setToast({ msg:'Paste your CSV URL first.', type:'err' }); return;
    }
    setLoadingH(true);
    try {
      const h = await fetchSheetHeaders(local.csvUrl);
      setHeaders(h);
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', padding:24 }}>
      <div style={{ maxWidth:800, margin:'0 auto', display:'flex', flexDirection:'column', gap:20 }}>
        ...
        {/* ── Google Sheet CSV ── */}
        <div className="card">
          <h2 style={{ fontSize:20, marginBottom:4 }}>Google Sheet — Form Responses</h2>
          <p style={{ fontSize:12, color:'#a3a3a3', marginBottom:16 }}>
            No API key needed. Just publish your sheet as CSV and paste the URL.
          </p>
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ fontSize:11, color:'#3a3a3a', marginTop:6, lineHeight:1.8 }}>
              How to get this URL:<br/>
              1. Open your Google Sheet (Form → Responses → Sheets icon)<br/>
              2. <b style={{color:'#a3a3a3'}}>File → Share → Publish to web</b><br/>
              3. Change dropdowns to <b style={{color:'#a3a3a3'}}>Sheet 1</b> and <b style={{color:'#a3a3a3'}}>CSV</b><br/>
              4. Click <b style={{color:'#a3a3a3'}}>Publish</b> → copy the URL → paste it here
            </div>
          </div>
          <button className="btn btn-ghost" onClick={loadHeaders} disabled={loadingH||!local.csvUrl}>
