...
// Inline cross-reference component
function CrossRef({matchRow, formTeam}) {
  if (!matchRow && !formTeam) return <span style={{color:'#3a3a3a',fontSize:11}}>No data on either side.</span>;
  if (!matchRow) return <span style={{color:'#3a3a3a',fontSize:11}}>No match data to compare yet.</span>;
  if (!formTeam) return <span style={{color:'#3a3a3a',fontSize:11}}>No scouting form data to compare.</span>;

  const items = [];

  if (matchRow.autoNear && formTeam.autoClose)  items.push({type:'confirm', msg:'Close auto confirmed across both sources'});
  if (matchRow.autoNear && !formTeam.autoClose) items.push({type:'conflict',msg:'Match shows close auto — form says no close auto'});
  if (matchRow.autoFar  && formTeam.autoFar)    items.push({type:'confirm', msg:'Far auto confirmed across both sources'});
  if (matchRow.autoFar  && !formTeam.autoFar)   items.push({type:'conflict',msg:'Match shows far auto — form says no far auto'});
  if (matchRow.leave    && formTeam.leave)       items.push({type:'confirm', msg:'Leave confirmed'});
  if (matchRow.leave    && !formTeam.leave)      items.push({type:'conflict',msg:'Match shows leave — form says no leave'});

  if (matchRow.avgTpArt !== null && formTeam.avgTeleop) {
    const diff = Math.abs(matchRow.avgTpArt - parseFloat(formTeam.avgTeleop));
    if (diff > 4) items.push({type:'conflict',msg:`Teleop discrepancy: match avg ${matchRow.avgTpArt} vs form avg ${formTeam.avgTeleop}`});
    else          items.push({type:'confirm', msg:`Teleop volume consistent (~${matchRow.avgTpArt} artifacts)`});
  }

  if (matchRow.bestPark==='full')    items.push({type:'confirm', msg:'Full park observed in match'});
  if (matchRow.bestPark==='partial') items.push({type:'neutral', msg:'Partial park observed in match'});
  if (matchRow.bestPark==='none')    items.push({type:'neutral', msg:'No park observed in any match'});

  if (!items.length) items.push({type:'neutral',msg:'Not enough data to cross-reference yet.'});

  return (
    <div style={{fontSize:12,lineHeight:2}}>
      {items.map((item,i)=>(
        <div key={i} style={{
          color: item.type==='confirm'?'#22c55e':item.type==='conflict'?'#f97316':'#525252',
        }}>
          {item.type==='confirm'?'✓ ':item.type==='conflict'?'⚠ ':'· '}{item.msg}
        </div>
      ))}
    </div>
  );
