...
const QUESTIONS = [
  ...
  { key:'ballsAuto',  label:'How many balls shooting in auto?',          type:'num'  },
...
function mergeSheetData(prev, imported) {
  ...
  const updated = imported.map(imp => ({
    ...(byNum[imp.teamNumber] || {}),
    ...imp,
    teamName: byNum[imp.teamNumber]?.teamName || imp.teamNameFromForm || imp.teamName || '',
...
export default function FormTab({ teams, setTeams, settings, setToast }) {
  ...
  function submit() {
    ...
    setTeams(prev => {
      ...
      const prior = inRegistry ? (prev.find(t => t.teamNumber === activeTeamNum)?.matchCount || 0) : 0;
      const n = prior + 1;
      const avg = (old, nw) => {
      ...
      if (inRegistry) {
        ...
        // New team — add to registry
        const newTeam = {
          ...BLANK_TEAM,
          teamNumber:  activeTeamNum,
          teamName:    manualName.trim() || '',
          matchCount:  1,
          hasAuto:     response.hasAuto    ?? false,
          autoClose:   response.autoClose  ?? false,
          autoFar:     response.autoFar    ?? false,
          capacity:    response.capacity   || '',
          ballsAuto:   response.ballsAuto  || '',
          leave:       response.leave      ?? false,
          avgAuto:     parseFloat(response.avgAuto)    || '',
          highAuto:    parseFloat(response.highAuto)   || '',
          avgTeleop:   parseFloat(response.avgTeleop)  || '',
          highTeleop:  parseFloat(response.highTeleop) || '',
          hasGoal:     response.hasGoal    ?? false,
          goalDetail:  response.goalDetail || '',
          endgame:     response.endgame    || '',
          source: 'form',
        };
        return [...prev, newTeam]
          .sort((a,b) => (parseInt(a.stateRank)||999)-(parseInt(b.stateRank)||999));
        ...
        <div style={{flex:1,overflowX:'auto',overflowY:'auto'}}>
          {!scoutedTeams.length
            ...
            : <table style={{borderCollapse:'collapse',whiteSpace:'nowrap'}}>
                <thead><tr>
                  <th style={th()}>#</th><th style={th()}>Team</th><th style={th()}>Subs</th>
                  <th style={th('#f97316')}>Auto</th><th style={th('#f97316')}>Close</th>
                  <th style={th('#f97316')}>Far</th><th style={th('#f97316')}>Leave</th>
                  <th style={th('#f97316')}>Balls Auto</th><th style={th('#f97316')}>Avg Auto</th><th style={th('#f97316')}>High Auto</th>
                  <th style={th('#60a5fa')}>Capacity</th><th style={th('#60a5fa')}>Avg TP</th><th style={th('#60a5fa')}>High TP</th>
                  <th style={th('#60a5fa')}>Has Goal</th><th style={th('#60a5fa')}>Goal</th>
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:8}}>
              <input value={manualNum} onChange={e=>setManualNum(e.target.value)}
                placeholder="Team # e.g. 12345"
                style={{maxWidth:150,fontFamily:'var(--font-mono)',fontSize:14}}/>
