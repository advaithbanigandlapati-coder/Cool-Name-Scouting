...
import { ClipboardCheck, Check, Table2, ChevronDown, ChevronUp, Zap } from 'lucide-react';
import { BLANK_TEAM } from '../constants.js';
import { fetchTeamStats } from '../api/ftcscout.js';
import { analyzeTeam } from '../api/claude.js';
...
function CrossRef({matchRow, formTeam}) {
  if (!matchRow && !formTeam) return <div style={{color:'#3a3a3a',fontSize:11}}>No data on either side yet.</div>;
  if (!matchRow) return <div style={{color:'#3a3a3a',fontSize:11}}>No match data yet — log a match form to compare.</div>;
  if (!formTeam) return <div style={{color:'#3a3a3a',fontSize:11}}>No scout form data yet — fill in the Scout tab to compare.</div>;
  ...
  if (matchRow.avgTpArt != null && formTeam.teleopArtifacts) {
    const diff = Math.abs(matchRow.avgTpArt - parseFloat(formTeam.teleopArtifacts));
    if (diff > 5) conflicts.push(`Teleop artifacts: match avg ${matchRow.avgTpArt} vs scouted ${formTeam.teleopArtifacts}`);
    else confirms.push(`Teleop volume consistent (~${matchRow.avgTpArt} artifacts)`);
  }

  if (matchRow.bestPark === 'full'    && formTeam.parkType === 'Partial') conflicts.push('Match shows full park, scout says partial');
  if (matchRow.bestPark === 'full'    && formTeam.parkType === 'Full')    confirms.push('Full park confirmed');
  if (matchRow.bestPark === 'partial' && formTeam.parkType === 'Full')    conflicts.push('Scout says full park but only partial seen in matches');

  return (
    <div style={{fontSize:11,lineHeight:1.7}}>
      {confirms.map((c,i) => <div key={i} style={{color:'#22c55e'}}>✓ {c}</div>)}
      {conflicts.map((c,i) => <div key={i} style={{color:'#f97316'}}>⚠ {c}</div>)}
      {!confirms.length && !conflicts.length && <div style={{color:'#3a3a3a'}}>No discrepancies found.</div>}
    </div>
  );
}
...
export default function MatchFormTab({ teams, setTeams, mine, matchEntries, setMatchEntries, setToast }) {
  ...
  async function analyzeOne(team) {
    setAnalyzing(team.teamNumber);
    try {
      const result = await analyzeTeam(team, mine, teams);
      setTeams(prev => prev.map(t => t.teamNumber===team.teamNumber ? {...t,...result} : t));
      setToast({ msg:`✓ AI analyzed #${team.teamNumber}`, type:'ok' });
    } catch { setToast({ msg:`AI failed for #${team.teamNumber}`, type:'err' }); }
