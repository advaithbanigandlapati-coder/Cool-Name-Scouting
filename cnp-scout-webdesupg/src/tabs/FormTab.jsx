...
import { ClipboardList, Check, Wifi, WifiOff, Radio, Zap } from 'lucide-react';
...
export default function FormTab({ teams, setTeams, settings, mine, setToast }) {
  ...
  const [analyzing,    setAnalyzing]    = useState(null); // teamNumber being analyzed
  ...
  // ── AI analyze single team from scout data ────────────────────────────────
  async function analyzeOne(team) {
    ...
    try {
      ...
      setToast({ msg:`AI failed for #${team.teamNumber}`, type:'err' });
                  ...
                  {scoutedTeams.map((t,i)=>(
                    <tr key={t.teamNumber} style={{background:i%2===0?'#0c0c0c':'#0f0f0f'}}>
                      ...
                      <td style={{...td,textAlign:'center'}}>
                        <button onClick={()=>analyzeOne(t)}
                          disabled={analyzing===t.teamNumber}
                          className="btn btn-ghost"
                          style={{fontSize:10,padding:'2px 8px',color:'#f97316',
                            background:t.tier?'rgba(249,115,22,0.1)':'transparent'}}>
                          {analyzing===t.teamNumber
                            ? <div className="spinner" style={{width:10,height:10}}/>
                            : t.tier ? `${t.tier}` : <><Zap size={10}/> AI</>}
                        </button>
                      </td>
                    </tr>
                  ))}
